...
# The second path is the bidsmap file for your study
...
# and execute yje xnat2bids command with the given inputs. The `-B ${data_dir}` makes that
# available inside the singularity container
singularity exec -B ${data_dir} \
    ${simg} xnat2bids ${XNAT_SESSION} ${bids_root_dir} \     			                                \
    -u ${XNAT_USER} -p "${XNAT_PASSWORD}" \
    -f ${bidsmap_file}